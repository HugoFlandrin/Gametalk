...
document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('http://localhost:8081/api/getLikes', {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json'
        }
        ...
        data.forEach(async like => {
            if (userID == like.user_id) {
                if (parseInt(like.post_id) != 0) {
                    commentsLiked.push(like.post_id)
                } else if (parseInt(like.topic_id) != 0) {
                    postsLiked.push(like.topic_id)
                }
            }
        });
    ...
    fetch('http://localhost:8081/api/getposts', {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json'
        }
        ...
        const cardContainer = document.querySelector('.cardContainer2');
        if (cardContainer) {
            data.forEach(async post => {
                for (let i = 0; i < commentsLiked.length; i++) {
                    if (commentsLiked[i] == post.id) {
                        let newCard;
                        newCard = await createCardElement(post.id, post.body, post.created_by, post.title, post.CategoryName, post.CreatedAtFormatted );
                        cardContainer.appendChild(newCard);
                    }
